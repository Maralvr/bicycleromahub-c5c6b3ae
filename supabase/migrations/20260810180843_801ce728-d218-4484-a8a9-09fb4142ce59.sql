-- 1. Nicer greeting name + absolute app link in guide notification emails
...
BEGIN
  ...
  -- Turn things like "renato.nobilio" or "maral@x.com" into "Renato"
...
-- 2. Scope realtime broadcast access to the single shared, non-sensitive topic
DROP POLICY IF EXISTS "Authenticated can read app realtime channels" ON realtime.messages;
CREATE POLICY "Authenticated can read shared shift changes channel"
ON realtime.messages
FOR SELECT
TO authenticated
USING (realtime.topic() = 'shifts-changes');