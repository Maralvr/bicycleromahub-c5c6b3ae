...
function DashboardPage() {
  ...
  const stats = [
    { label: t.dashboard.shiftsToday, value: todayShifts.length, sub: "+2 vs yesterday", icon: CalendarRange, accent: true },
    { label: t.dashboard.pendingAccept, value: pending.length, sub: "Awaiting response", icon: Clock },
    { label: t.dashboard.activeStaff, value: `${activeStaff}/${staff.length}`, sub: "On the clock", icon: Users2 },
    { label: t.dashboard.openTasks, value: openTasks.length, sub: `${tasks.length - openTasks.length} done today`, icon: ClipboardCheck },
  ];
        ...
        {stats.map((s) => (
          ...
          >
            <div className="flex items-start justify-between mb-4">
              <div className={`inline-flex h-10 w-10 items-center justify-center rounded-xl ${s.accent ? "bg-primary text-primary-foreground" : "bg-muted text-foreground/70"}`}>
          ...
          <div className="flex items-start gap-4 relative">
            ...
            <div className="flex-1 min-w-0">
              ...
              <div className="mt-3 space-y-2">
                {unassigned.map((s) => (
                  <div key={s.id} className="flex items-center justify-between bg-card rounded-lg p-3 border border-border/60 hover:border-primary/40 transition-colors">
        ...
        <Card className="p-5 lg:col-span-2">
          ...
          <div className="space-y-1">
            {shifts.slice(0, 5).map((s) => {
              ...
              return (
                <div key={s.id} className="flex items-center gap-3 p-3 -mx-3 rounded-lg hover:bg-muted/50 transition-colors group cursor-pointer">
                  <div className="h-12 w-12 rounded-xl bg-gradient-to-br from-primary to-primary-glow flex flex-col items-center justify-center text-primary-foreground flex-shrink-0">
                    <div className="text-[9px] uppercase tracking-wider opacity-80 leading-none">{s.startTime.slice(0, 2)}h</div>
                    <div className="text-sm font-bold leading-tight">{s.startTime.slice(3, 5)}</div>
                  </div>
                  <div className="flex-1 min-w-0">
                    <div className="font-medium text-sm truncate group-hover:text-primary transition-colors">{s.tourName}</div>
                    <div className="text-xs text-muted-foreground flex items-center gap-1 mt-0.5">
                      <MapPin className="h-3 w-3 flex-shrink-0" /> <span className="truncate">{s.meetingPoint}</span>
                    </div>
                  </div>
                  <div className="flex flex-col items-end gap-1 flex-shrink-0">
                    {guide ? (
                      <div className="flex items-center gap-1.5">
                        ...
                        <span className="text-xs font-medium hidden sm:inline">{guide.name.split(" ")[0]}</span>
                      </div>
                    ) : (
            ...
            {updates.map((u) => {
              ...
              return (
                <div key={u.id} className="flex gap-3">
                  <Avatar name={author?.name || "Admin"} initials={author?.avatar || "AD"} size="sm" />
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-1.5 text-xs">
                      <span className="font-semibold text-foreground">{author?.name || "Admin"}</span>
