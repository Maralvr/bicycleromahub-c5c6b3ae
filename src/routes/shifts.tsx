...
import { fetchBusyGuides } from "@/lib/guide-conflicts";
...
function ShiftsPageRouter() {
  ...
  if (authEarlyReturn) return null;
  if (isRentalStaff) {
    return (
    ...
    <div className="mt-4 p-3 rounded-lg border border-border/60 bg-muted/30">
      ...
      <div className="flex flex-wrap items-end gap-2">
        ...
        <div className="flex-1 min-w-[200px] space-y-1">
          ...
          <RateTitleField id={`ov-lang-${shift.id}`} value={rateTitle} onChange={setRateTitle} className="h-9 text-xs" bokunProductId={shift.bokunProductId ?? null} bokunRateId={shift.bokunRateId ?? null} tourName={shift.tourName ?? null} />
        </div>
